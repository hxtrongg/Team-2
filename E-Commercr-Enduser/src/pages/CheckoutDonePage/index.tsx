...
const CheckoutDonePage = () => {
  ...
  const navigate = useNavigate();
    ...
    const { items, total, itemCount, placeOrder} = useCartStore();
    ...
    items.map((item) => {
      return(
        ...
        <div className="flex flex-wrap items-center justify-start gap-4 px-4 mt-6 ">
          <Link to={'/products'} className="w-full px-4 py-2 text-blue-500 border border-blue-500 rounded-md md:w-auto hover:text-gray-100 hover:bg-blue-600 dark:border-gray-700 dark:hover:bg-gray-700 dark:text-gray-300">
            Go back shopping
          </Link>
          <Link to={'/customers/orders/ordersdetail'} className="w-full px-4 py-2 bg-blue-500 rounded-md text-gray-50 md:w-auto dark:text-gray-300 hover:bg-blue-600 dark:hover:bg-gray-700 dark:bg-gray-800">
            View career details
          </Link>
        </div>
      </div>
