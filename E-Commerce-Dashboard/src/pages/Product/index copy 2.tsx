...
const ProductPage = () => {
  ...
  const fetchDelete = async (objectID: string) => {
    return await axiosClient.delete(config.urlAPI + "/v1/products/" + objectID);
  };
  ...
  let productData = [];
  if (queryProducts && queryProducts.data) {
    const {
      data: {
        data: {
          data: { products },
  ...
  return (
    <>
      ...
      <Table dataSource={productData} rowKey={"name"}>
        <Column title="Tên sản phẩm" dataIndex="name" key="name" />
        <Column
          title="Mô tả"
          dataIndex="meteDescription"
          key="meteDescription"
        />
